...
class ProductController {
  static async importProducts(req, res) {
    ...
    if (!Array.isArray(partNumbers) || partNumbers.length === 0) {
      return res.status(400).json({ error: 'partNumbers phải là mảng chuỗi và không rỗng' });
    }
    ...
    try {
      ...
      if (parallel) {
        // Chạy song song
        results = await Promise.all(partNumbers.map(pn => DigikeyService.fetchProduct(pn)));
      } else {
    ...
    static async listProducts(req, res) {
        ...
        const { count, rows } = await DigikeyService.list({ page, limit, search });
    ...
    if (!Array.isArray(partNumbers) || !partNumbers.length) {
      return res.status(400).json({ error: 'partNumbers phải là mảng và không rỗng' });
    }

    try {
      const products = await DigikeyService.fetchByManufacturerPNs(partNumbers);
      if (!products.length) {
        return res.status(404).json({ error: 'Không tìm thấy sản phẩm nào với partNumbers đã cho' });
      }

      const allFields = Object.keys(Product.rawAttributes).filter(f => f !== 'id');
